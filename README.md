...
- **Relativistic laser model ("photon rocket")**: SR and GR effects are strictly accounted for:
  ...
  - During flight, photons experience the cosmological redshift factor `a(t_emit)/a_eff(t_arrival)`. In the **hybrid model** (FRW background + central point mass, McVittie-like leading order) this is the standard FRW redshift `a(t_emit)/a(t_arrival)` as long as the photon stays outside the central BH's sphere of influence `r_*³ = 3·r_S/Λ` (where Schwarzschild and `Λ`-terms balance in `f(r) = 1 − r_S/r − Λr²/3`). Inside `r_*` the local metric is effectively static Schwarzschild–de Sitter, and the Killing energy `E_∞` is the exact invariant of the radial null geodesic — so the FRW factor is **frozen** at its value at the moment of crossing `r_*`. Practically, each photon packet stores `a_freeze`, set on first entry into `r_*` (or into the inner apparent horizon, whichever is larger), and absorption uses `m_emit·a_emit/a_freeze` instead of `m_emit·a_emit/a(t_now)`. This correction matters near the Nariai limit: as `M_BH → M_N`, `r_*` grows to `~r_N` and a photon spends a non-trivial fraction of its trip in the SdS-dominated zone; without the freeze the delivered energy would be systematically underestimated.
